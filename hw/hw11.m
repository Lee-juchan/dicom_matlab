...
for roi = 1:nROIs_selected
    ...
    for ss = 1:nSlice
        ...
        z_slice = contourData_slice(1, 3); % 한 slice의 z값은 모두 값기에, 첫번째 z값 선택
        z_roi = [z_roi; z_slice]; % 길이가 다른 이 놈을 저장하기 위해, roiData => structure 사용
    end
...
% plot contour for all slices (zz)
fig = figure('color', 'w');
...
for zz = -1064.5:-1056.5 % z범위 안다고 가정
    ...
    % 한 slice에 각 contour () 그리기
    for roi = 1:nROIs_selected
        contourData_selected = roiData(roi).ContourData;
        color_selected = roiData(roi).Color;
        z_roi = roiData(roi).z_roi;

        % get contour data for each slice (z)
        z_roi_index = find(z_roi == zz); % ROI의 전체 z좌표 == 현 slice의 z좌표 (CT slice 찾는 것과 유사)
